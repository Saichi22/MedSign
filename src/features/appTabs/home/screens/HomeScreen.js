...
// ─── Frame / model constants (must match model input shape) ───────────────────
...
export default function HomeScreen() {
  ...
  // ── Frame processor (mirrors + throttles, matches TranslatorScreen) ───────
  const frameProcessor = useFrameProcessor(
    (frame) => {
      ...
      if (resized != null) {
        // Horizontal flip to match front-camera mirroring expected by the model
  ...
  // ── Append new predictions to history while recording ────────────────────
  useEffect(() => {
